...
public class Clinica {
	...
	public void addMedicinaPaciente(String pPac, String nMed) {
        ...
        boolean salir = false;
        Node<Medicamento> actM = this.laListaMed.first;
        while (!salir && !actM.equals(this.laListaMed.last)) { //BUSQUEDA MEDICAMENTO
            if (actM.data.getNombre().equals(nMed)) {
                salir = true;
                actM.data.setCantidad(actM.data.getCantidad()+1);
            }else{
                actM = actM.next;
            }
        }
        ...
        if (!salir) {
            Medicamento nuevo = new Medicamento(nMed, 1);
            laListaMed.addToRear(nuevo);
        }
        salir = false;

        Node<Paciente> actP = this.laListaPac.first;
        while (!salir) {  //BUSQUEDA PACIENTE
            if (actP.data.getNombre().equals(pPac)) {
                //actP.data.getLista().addToRear();
                salir = true;
            }else{
                actP = actP.next;
            }
